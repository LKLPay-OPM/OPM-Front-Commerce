...
export async function load({ url, params }) {
  ...
  try {
    const response = await axiosDevicesClient.get(`/transaction/${date}`, { params: { 
    ...
    if (validQueryFilters.includes(filters.filter)) return { date, filters, response: response.data?.response };
  } catch (e) {
