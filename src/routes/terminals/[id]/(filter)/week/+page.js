...
export async function load({ url, params }) {
  ...
  try {
    const response = await axiosDevicesClient.get(`/transaction`, { params: { idTerminal, filter, start, end } });
    if (validQueryFilters.includes(filter))
      return { idTerminal, filter, start, end, response: response.data?.response };
  } catch (e) {
