        ...
        .then((value) => {
          if ($loggedInUser.accountType === "basic") {
            rates.push({
              ...
              value:
                Math.round(
                  (value.ratesBusinessType[$loggedInUser.businessLine].amex +
                    value.rateLklPay -
                    $loggedInUser.ratesDiscount.amex) *
                    100
                ) / 100,
            });
            rates.push({
              ...
              value:
                Math.round(
                  (value.ratesBusinessType[$loggedInUser.businessLine].credit +
                    value.rateLklPay -
                    $loggedInUser.ratesDiscount.credit) *
                    100
                ) / 100,
            });
            rates.push({
              ...
              value:
                Math.round(
                  (value.ratesBusinessType[$loggedInUser.businessLine].debit +
                    value.rateLklPay -
                    $loggedInUser.ratesDiscount.debit) *
                    100
                ) / 100,
            });
            console.log(rates);
    ...
    <div class="transactions">
      <div class="transaction-form">
        <form on:submit|preventDefault={handleCreateTransaction}>
          ...
          <Input
            label="No. de Serie de la Terminal"
            id="transaction-id"
            bind:value={terminalData.serialNumber}
          ...
          <Input
            ...
            bind:value={transactionForm.cardNumber}
            className="txt-field normal"
            type="text"
          />
          <Input
            ...
            bind:value={transactionForm.total}
            className="txt-field normal"
            type="number"
          />
          {#if rates.length > 0}
